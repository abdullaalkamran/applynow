...
import { COUNSELLORS, AGENTS, ADMISSION_OFFICERS } from "../data/mockData";
...
export const ASSIGNABLE_ROLES: Record<TaskRole, TaskRole[]> = {
  counsellor: ["agent", "student", "admin", "admission"],
  admin: ["counsellor"],
  agent: ["student"],
  student: [],
  admission: [],
...
export function recipientsFor(from: TaskPerson): TaskPerson[] {
  if (from.role === "counsellor") {
    ...
    const admissionPeople: TaskPerson[] = ADMISSION_OFFICERS.map((o) => ({ id: o.id, role: "admission", name: o.name }));
    return [...studentPeople, ...agentPeople, ...admissionPeople, ADMIN_PERSON];
  }
