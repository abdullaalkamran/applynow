...
export default function AgentVisaCompliance() {
  const navigate = useNavigate();
  const students = loadAgentStudents();
  const rows = getAllApplications()
    .filter((a) => students.some((s) => s.id === a.studentId) && agentStageFor(a.status) === "Visa")
    .map((app) => ({ app, student: students.find((s) => s.id === app.studentId)! }));

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-base font-semibold text-slate-900">Visa &amp; Compliance</h1>
        <p className="mt-1 text-xs text-slate-500">{rows.length} student{rows.length === 1 ? "" : "s"} currently working through their visa.</p>
      </div>
      ...
      <div className="space-y-2.5">
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex shrink-0 items-center gap-2">
                {student.riskFlag && student.riskFlag !== "none" && (
                  <Badge tone={student.riskFlag === "high" ? "red" : "amber"}>{student.riskFlag === "high" ? "High risk" : "Watch"}</Badge>
                )}
        ...
        {rows.length === 0 && (
          <p className="py-10 text-center text-xs text-slate-400">No students are in the visa stage right now.</p>
        )}
      </div>
    </div>
  );
